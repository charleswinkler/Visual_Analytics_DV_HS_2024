...
# Linke Spalte: Gauge und Auslastungsdiagramm
with col1:
    # Gauge für Parkhaus-Auslastung
    ...
    # Zeitverlauf der Parkhaus-Auslastung
    st.write("### Auslastung der Parkhäuser im Zeitverlauf")
    selected_parkhaus_1 = st.selectbox('Wähle das erste Parkhaus:',
                                       ['Keine Auswahl'] + sorted(df_parkhaus['title'].unique()), key="park1")
    selected_parkhaus_2 = st.selectbox('Wähle das zweite Parkhaus:',
                                       ['Keine Auswahl'] + sorted(df_parkhaus['title'].unique()), key="park2")

    fig_line = px.line()
    if selected_parkhaus_1 != 'Keine Auswahl':
        fig_line.add_scatter(
            x=df_parkhaus[df_parkhaus['title'] == selected_parkhaus_1]['published'],
    ...
    if selected_parkhaus_2 != 'Keine Auswahl':
        fig_line.add_scatter(
            x=df_parkhaus[df_parkhaus['title'] == selected_parkhaus_2]['published'],
    ...
    fig_line.update_layout(title="Auslastung der Parkhäuser im Zeitverlauf", xaxis_title="Zeitpunkt",
                           yaxis_title="Belegte Plätze")
    st.plotly_chart(fig_line)

# Rechte Spalte: Säulendiagramm und Heatmap
with col2:
    # Säulendiagramm für Klimadaten
    st.write("### Klima-Temperaturverlauf")
    df_klima = df_3[df_3['Kategorie'] == 'Klima']
    selected_klima_title = st.selectbox('Wähle ein Klima-Objekt:',
                                        ['Keine Auswahl'] + sorted(df_klima['title'].unique()))
    df_klima_filtered = df_klima if selected_klima_title == 'Keine Auswahl' else df_klima[
        df_klima['title'] == selected_klima_title]
    ...
    # Heatmap der Temperaturdaten mit Standort- und Datumsfilter
    st.write("### Heatmap der Temperatur")

    # Datum- und Standortfilter
    ...
    klima_standort = st.selectbox("Wähle einen Klima-Standort:",
                                  ['Alle Standorte'] + sorted(df_3[df_3['Kategorie'] == 'Klima']['title'].unique()))

    # Daten filtern
    filtered_df = df_3[(df_3['date'] >= start_date) & (df_3['date'] <= end_date) & (df_3['Kategorie'] == 'Klima')]

    if klima_standort != 'Alle Standorte':
    ...
    fig_heat = px.density_heatmap(heatmap_data, x="date_hour", y="title", z="meta_airtemp",
                                  color_continuous_scale="Viridis",
                                  labels={'meta_airtemp': 'Temperatur (°C)', 'date_hour': 'Datum und Stunde',
                                          'title': 'Klimastation'})
    fig_heat.update_layout(title="Temperatur-Heatmap nach Datum, Stunde und Klimastation",
                           xaxis_title="Datum und Stunde", yaxis_title="Klimastation", xaxis_tickangle=-45)
